# Runbook — sdkwork-tts 部署 / 升级 / 回滚（中文）

适用环境：`development|test|staging|demo|production`。所有命令默认在本机 WSL 执行，远程主机加 `--host ssh://[user@]host[:port]`。镜像参考：`registry.sdkwork.com/apps/sdkwork-tts-standalone:0.1.0`（tag 取自 `sdkwork.app.config.json` → `release.currentVersion`）。
...
install 会同步 bundle 到 `/opt/deploy/sdkwork-tts/bundle`，加载镜像，按实例启动并等待健康门禁（`/healthz`）。
