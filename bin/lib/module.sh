#!/usr/bin/env bash
# module.sh — sdkwork-tts bin/ wiring (MODULE_BIN_SPEC.md §3).
...
SDKWORK_MODULE_ID="sdkwork-tts"
SDKWORK_IMAGE_NAME="sdkwork-tts-standalone"
  ...
  case "${app_type}" in
    ...
    *)
      sdkwork_die "${SDKWORK_BIN_E_ENV}" \
        "app type '${app_type}' has no wired build for sdkwork-tts; extend sdkwork_build_app with the repository's canonical runner (declared: ${SDKWORK_APP_TYPES})" ;;
  esac
}
...
sdkwork_package_app() {
  ...
  sdkwork_die "${SDKWORK_BIN_E_STATE}" \
    "sdkwork-tts has no canonical release packager wired yet; implement sdkwork_package_app against the repository's packaging command (MODULE_BIN_SPEC.md §4.4)"
}
...
sdkwork_installer_app() {
  ...
  sdkwork_die "${SDKWORK_BIN_E_STATE}" \
    "sdkwork-tts has no native installer builder wired yet; implement sdkwork_installer_app against the repository's installer commands (MODULE_BIN_SPEC.md §4.9; platforms: windows|linux|macos|android|ios)"
}
...
sdkwork_deploy_app() {
  ...
  sdkwork_die "${SDKWORK_BIN_E_STATE}" \
    "sdkwork-tts has no application deployment channel wired yet; implement sdkwork_deploy_app (host-native install via bin/apps-package artifacts, MODULE_BIN_SPEC.md §4.5)"
}
